...
export class BezierPath {
    ...
    constructor(segments: BezierSegment[]) {
        ...
        const numSegSamples = (seg: BezierSegment) => Math.max(4, Math.ceil(seg.getTotalLength() / BezierSegment.sampleSpacing()));
        ...
        for (let i = 1; i < this.samples.length; i++) {
            ...
            if (prev.segIdx !== next.segIdx && (this.segments[prev.segIdx].D.x !== this.segments[next.segIdx].A.x || this.segments[prev.segIdx].D.y !== this.segments[next.segIdx].A.y)) {
